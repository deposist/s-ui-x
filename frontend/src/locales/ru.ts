export default {
  ...
  form: {
    ...
    cannotSave: {
      ...
      capabilityUnavailable: "Этот тип недоступен в запущенной сборке ядра",
    },
    sections: {
      basic: "Основное",
  ...
  guidance: {
    ...
    protocols: {
      vless: "Используйте корректный UUID клиента. Для Vision нужны совместимые настройки TLS и транспорта; оставьте flow пустым, если эти условия не настроены намеренно.",
      vmess: "Используйте корректный UUID клиента и Alter ID 0. Оставьте автоматический режим security, если совместимость не требует конкретный шифр.",
      trojan: "Используйте стойкий уникальный пароль клиента. Обычно требуется TLS; настраивайте fallback только при наличии соответствующего сервиса.",
      shadowsocks: "Выберите метод, поддерживаемый всеми клиентами, и задайте стойкий уникальный пароль. Не включайте плагин, если он не требуется на обеих сторонах.",
      socks: "Аутентификация необязательна. Для современных клиентов выбирайте SOCKS5 и открывайте listener только доверенным сетям.",
      http: "Аутентификация необязательна. Перед публикацией HTTP-прокси за пределами доверенной сети включите TLS.",
    },
  },
  loading: "Загрузка...",
  confirm: "Вы уверены?",
  ...
  login: {
    ...
    forcePasswordReset: "Перед продолжением необходимо изменить пароль.",
  },
  menu: {
    logout: "Выйти",
    language: "Язык",

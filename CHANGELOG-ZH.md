...
- 面板识别旧的数字 self-update 标记，将其转换为当前 JSON 格式，并继续正常的恢复流程。
- 反复无法启动的新二进制仍会在同样的阈值下回滚到备份。没有备份时，启动仍会停止。
- release notes 中加入了针对已陷入重启循环的服务器的手动恢复步骤。

完整 release notes: [`docs/releases/v1.5.12-beta3.md`](docs/releases/v1.5.12-beta3.md)。
...
- 官方内核：仅使用 `github.com/sagernet/sing-box v1.13.15` 构建。所有 fork 补丁、扩展协议标签和第三方构建标志均已移除。
- Capability 清单驱动所有层：后端验证、前端编辑器、导入、doctor 诊断和运行时检查都会在持久化之前拒绝不支持的类型。历史行保持可检查和可编辑，但仅当类型与当前构建匹配时才允许。
- Self-update 现在是二进制加数据库的单一事务。替换任何文件之前，面板会写入一个 JSON 事务标记，其中包含候选文件、备份和 `VACUUM INTO` 数据库快照的 SHA-256 摘要。恢复在迁移之前运行，并在状态格式错误或身份不匹配时 fail closed。
- 修复了更新流程中的状态泄漏：pending marker 的 `os.Stat` 错误可能导致服务永久阻塞。现在所有错误路径都会在返回前重置更新状态。
- 支付订单保存不可变的授权天数、流量和快照版本。没有快照的旧订单会进入永久人工审核状态，并保留提供商证据。
- CryptoBot 发票身份成为一等公民。创建过程是串行且 fail-closed 的。对账优先处理已支付的重复项，取消活动同级项，并在重启之间保留失败的取消重试。
- Classic 和 Nexus 共享菜单元数据和调色板感知主题。实体抽屉拒绝空白标识、无效端口、缺失 TLS 和不可用的运行时能力，并显示可见的本地化原因。
- 所有六种发布语言现在暴露相同的叶键结构。缺失的翻译在运行时回退到英语，但会在包中具体化，因此测试会拒绝结构漂移。
- 会话重新生成、SQLite 会话、CSRF、WebSocket 源、速率限制、cookie 不变量和备份机密性由重点竞态测试覆盖。

完整 release notes: [`docs/releases/v1.5.12-beta1.md`](docs/releases/v1.5.12-beta1.md)。

...
No database migration or configuration change is needed. The backend save path is unchanged.
...
The new drawer opens from a button in both the Rules and DNS pages. RU and ZH are separate cards. Each card has a direction switch: Direct sends regional domains through your direct outbound, Through proxy sends them through your proxy outbound. You pick proxy and direct outbounds once, shared by both regions.

When you click Preview changes, the drawer shows what will be added, what will change, and what custom rules will be kept. If a direction is Through proxy, the preview warns about possible DNS leak and route exposure risk. You can cancel from the preview without saving anything.
...
The preset catalog went from three overlapping presets to four: ru-direct, ru-proxy, zh-direct, zh-proxy. Each preset-managed rule set and DNS server uses a deterministic tag name (for example `preset-ru-proxy-blocked`). No custom metadata fields are written into the sing-box config; the panel detects and cleans up preset-managed items by tag name only.
...
Новая панель открывается кнопкой на обеих страницах, Rules и DNS. RU и ZH — отдельные карточки. В каждой карточке есть переключатель направления: Direct отправляет региональные домены через direct outbound, Through proxy — через proxy outbound. Proxy и direct outbound выбираются один раз и shared для обоих регионов.
...
Каталог пресетов изменился с трёх пересекающихся на четыре: ru-direct, ru-proxy, zh-direct, zh-proxy. Каждый пресет-управляемый rule set и DNS-сервер использует детерминированное имя тега (например `preset-ru-proxy-blocked`). В конфиг sing-box не записываются нестандартные metadata-поля; панель обнаруживает и удаляет пресет-элементы только по именам тегов.
